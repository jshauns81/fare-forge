...
export function getExtras(weekStart) {
  return getDb()
    .prepare('SELECT id, label FROM market_extras WHERE week_start = ? ORDER BY id')
    .all(weekStart)
...
// Forge recipes (no source URL) tagged "starter" — delete freely. A meta
// marker records that seeding happened, so deleting them all doesn't bring
// them back on the next restart.
function seedIfEmpty(db) {
  if (db.prepare("SELECT value FROM meta WHERE key = 'starters_seeded'").get()) return;
  const markSeeded = () =>
    db.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('starters_seeded', '1')").run();
  const n = db.prepare('SELECT COUNT(*) AS n FROM recipes').get().n;
  if (n > 0) {
    markSeeded(); // pre-marker database that already has recipes
    return;
  }
